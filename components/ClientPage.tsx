...
export default function ClientPage() {
  ...
  // Basic input sanitization that preserves apostrophes
  const sanitizeData = (data: any): any => {
    if (typeof data === 'string') {
      // Remove potential XSS vectors but preserve apostrophes
      return data
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
        .replace(/&/g, '&amp;')
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Decode HTML entities in the sanitized data
      if (sanitizedIdea.tagline) {
        sanitizedIdea.tagline = decodeHtmlEntities(sanitizedIdea.tagline);
      }
      if (sanitizedIdea.tweet) {
        sanitizedIdea.tweet = decodeHtmlEntities(sanitizedIdea.tweet);
      }
      ...
      if (fallbackIdeas.length > 0) {
        ...
        if (fallbackIdea.tagline) {
          fallbackIdea.tagline = decodeHtmlEntities(fallbackIdea.tagline);
        }
        if (fallbackIdea.tweet) {
          fallbackIdea.tweet = decodeHtmlEntities(fallbackIdea.tweet);
        }
        
        setStartupIdea(fallbackIdea);
      }
    } finally {
  ...
  const handleTweet = () => {
    ...
    try {
      // Create a simplified tweet without the vibeship.ai reference and hashtags
      let simplifiedTweet = startupIdea.tweet;
      
      // Remove the vibeship.ai reference and hashtags
      simplifiedTweet = simplifiedTweet
        .replace(/\s+at vibeship\.ai.*$/i, '')
        .replace(/\s+Try it:?\s+vibeship\.ai.*$/i, '')
        .replace(/\s+Generated by @VibeShipAI.*$/i, '')
        .replace(/\s+#\w+(\s+#\w+)*\s*$/i, '')
        .replace(/\s+@VibeShipAI.*$/i, '');
      
      // Ensure the tweet includes the startup name and tagline
      if (!simplifiedTweet.includes(startupIdea.name)) {
        simplifiedTweet = `${startupIdea.name}: ${startupIdea.tagline}`;
      }
      
      const tweetText = encodeURIComponent(simplifiedTweet);
